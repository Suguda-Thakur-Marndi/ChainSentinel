# Project Folder Structure & Architecture Overview

> **Project:** RiskWise / ChainSentinel  
> **Type:** Monorepo - Autonomous Multi-Agent Supply Chain Risk & Logistics Intelligence Platform  
> **Exclusions:** Build artifacts and dependency directories (`node_modules/`, `.venv/`, `.next/`, `__pycache__/`, `.git/`, `.pytest_cache/`, `.turbo/`)
...
| **Backend Core Stack** | Python 3.13, FastAPI, SQLAlchemy 2, Alembic, Pydantic v2, Redis, Celery |
| **Frontend Core Stack** | Next.js 15+ (App Router), React 19, TypeScript, Tailwind CSS, Lucide Icons |
| **Intelligence Stack** | LangGraph, AWS Bedrock (Claude 3.5 Sonnet), Scikit-Learn, NetworkX / Digital Twin |
| **Primary Workspaces** | `apps/api` (Backend), `apps/web` (Frontend), `docs` (Architecture & Specs), `.agents` (Agent Skills) |
...
| :--- | :--- | :--- |
| **`apps/api/`** | Backend App | Authoritative FastAPI backend service implementing REST APIs, multi-agent workflows, ML pipelines, RAG, digital twins, and risk scoring. |
| **`apps/web/`** | Frontend App | Next.js 15 modern frontend web application with interactive dashboards, risk matrices, and live logistics maps. |
| **`docs/`** | Documentation | Comprehensive architectural specifications, PRDs, database schemas, and phase-by-phase implementation plans (Phases 4–12). |
| **`.agents/`** | AI Skills | Repository-level agent skills, prompt specifications, design tokens, templates, and UI/UX catalogs. |
| **`AGENTS.md`** | Config / Rule | System rules and Next.js / framework conventions for AI coding assistants. |
| **`CLAUDE.md`** | Config / Rule | Architecture reference and commands for Claude-assisted development. |
| **`README.md`** | Documentation | Project overview, setup instructions, and feature guide. |
| **`.env.example`** | Config | Reference environment variable template for local setup. |
| **`risk-wise.pem`** | Key / Cert | Local security certificate / key credential (gitignored). |
| **`credentials.json`** | Key / Config | Local service account credential file (gitignored). |
...
### 1. `apps/api/` — Backend API & Intelligence Engine
...
- **`app/api/`**: HTTP layer:
  - `v1/endpoints/`: Modular REST endpoints for authentication, suppliers, logistics, risks, inventory, decisions, and agents.
...
- **`app/digital_twin/`**: Digital twin engine modeling physical supply chain entities (factories, ports, transit routes, inventory nodes) as an active graph.
- **`app/integrations/`**: Third-party external data ingestion adapters:
  - Telemetry: AISStream (vessels), OpenSky (air freight), TomTom (road traffic), OpenWeather (severe weather alerts).
...
- **`app/ml/`**: Machine learning sub-framework:
  ...
  - `inference/`: Prediction pipelines and low-latency inference servers.
  - `models/`: Trained model definitions (XGBoost, Random Forest, PyTorch).
  - `registry/`: Model versioning and checkpoint management.
  - `training/`: Training harnesses and evaluation pipelines.
- **`app/models/`**: SQLAlchemy ORM models representing entities (Users, Suppliers, Shipments, Nodes, Risks, Audits, Policies).
...
- **`tests/`**: Comprehensive test suite covering units, schemas, services, API endpoints, and authentication.
...
- **`app/`**: Next.js App Router root:
  - Route groups and pages for the supply chain dashboard, shipment tracking, risk monitor, agent command center, and simulation view.
...
*(This file was dynamically generated after repository reorganization)*
